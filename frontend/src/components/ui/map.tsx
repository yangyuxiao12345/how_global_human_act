...
function MarkerTooltip({
  ...
  useEffect(() => {
    ...
    // Keep tooltip open when cursor moves over the popup element
    ...
    const onPopupMouseLeave = () => scheduleHide();

    marker.getElement()?.addEventListener("mouseenter", showTooltip);
    marker.getElement()?.addEventListener("mouseleave", scheduleHide);

    // Attach listeners to the popup container once it's added to DOM
    const onTooltipOpen = () => {
      const el = tooltip.getElement();
      el?.addEventListener("mouseenter", onPopupMouseEnter);
      el?.addEventListener("mouseleave", onPopupMouseLeave);
    };
    tooltip.on("open", onTooltipOpen);

    return () => {
      if (hideTimeout) clearTimeout(hideTimeout);
      marker.getElement()?.removeEventListener("mouseenter", showTooltip);
      marker.getElement()?.removeEventListener("mouseleave", scheduleHide);
      tooltip.off("open", onTooltipOpen);
      tooltip.remove();
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [map]);
...
function MapControls({
  ...
  return (
    ...
    >
      {showZoom && (
        <ControlGroup>
          <ControlButton onClick={handleZoomIn} label="放大">
            <Plus className="size-4" />
          </ControlButton>
          <ControlButton onClick={handleZoomOut} label="缩小">
      ...
      {showFullscreen && (
        <ControlGroup>
          <ControlButton onClick={handleFullscreen} label="切换全屏">
...
function CompassButton({ onClick }: { onClick: () => void }) {
  ...
  return (
    <ControlButton onClick={onClick} label="将方向重置为正北">
      <svg
        ref={compassRef}
        viewBox="0 0 24 24"
